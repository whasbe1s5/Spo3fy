...
- Track metadata scrape block: reverted to the custom `Spo3fy` User-Agent because a standard browser User-Agent causes Spotify to serve a client-side SPA web player shell (which lacks SEO metadata tags), causing `ScrapeTrack` to always fail and return "Unknown Track".
- Playlist cover art extraction: added fallback selectors (preload image links and `img` tags) to retrieve the playlist cover art from the embed page, as that page does not contain standard `og:image` meta tags.
