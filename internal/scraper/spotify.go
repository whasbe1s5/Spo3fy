...
func ScrapeAlbum(albumID string) (*types.Album, error) {
	pageURL := spotifyBaseURL + "/album/" + albumID
	doc, err := fetchPage(pageURL)
	if err != nil {
	...
	if desc := metaContent(doc, "property", "og:description"); desc != "" {
		...
		if len(parts) >= 2 {
			if album.Name == "" {
				album.Name = strings.TrimSpace(parts[1])
			}
		}
		if len(parts) >= 3 {
	...
	if album.ImageURL == "" {
		album.ImageURL = types.DefaultCoverArtURL
	}

	// Get track IDs from music:song meta tags
	trackURLs := metaContents(doc, "name", "music:song")
	trackIDs := make([]string, 0, len(trackURLs))
	for _, tu := range trackURLs {
		if id := ExtractSpotifyID(tu); id != "" {
			trackIDs = append(trackIDs, id)
		}
	}

	album.TotalTracks = len(trackIDs)
	album.Tracks = make([]types.Track, 0, len(trackIDs))
...
// ScrapePlaylist scrapes playlist metadata + track list.
// Strategy 1: fetch embed page JSON (up to 100 tracks via regex).
// Strategy 2: scrape <meta name="music:song"> tags (30 tracks max).
func ScrapePlaylist(playlistID string) (*types.Playlist, error) {
	...
	if err == nil {
		if entries := tryParseEmbedJSON(body); len(entries) > 0 {
			...
			// Extract name / description from embed page
			if doc, docErr := goquery.NewDocumentFromReader(strings.NewReader(string(body))); docErr == nil {
				playlist.Name = extractPlaylistName(doc, body)
	...
	// Strategy 2: fallback to meta-tag scraping (30 tracks max)
	return fallbackScrapePlaylist(playlistID)
}

// fallbackScrapePlaylist scrapes playlist via meta tags (max 30 tracks).
